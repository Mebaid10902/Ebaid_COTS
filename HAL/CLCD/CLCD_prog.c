...
void CLCD_voidSendFloat(f32 value){
	uint8 trunc_val=(uint8)value;

	CLCD_voidSendNumber(trunc_val);
	uint8 decimal_value =(uint8)(((value -trunc_val))*100.0);
		CLCD_voidSendData('.');
		CLCD_voidSendNumber(decimal_value);
}
